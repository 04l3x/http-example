...
fn handle_connection(mut stream: TcpStream, router: &Router<'_>) {
    ...
    let routes = match http_request.method() {
        Method::Get => router.get(),
        Method::Post => router.post(),
    };

    let response = if let Some(route) = routes.get(path.as_str()) {
        route
    } else {
        &router.get()["/error"]
    };

    let response = response(http_request);
